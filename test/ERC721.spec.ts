...
describe('ERC721', () => {
    ...
    beforeEach(async () => {
        ...
        erc721 = await ethers.deployContract('ERC721', ['test name', 'test symbol'], owner)
    ...
    it('should be possible to safe mint a token by owner', async () => {
        ...
        await expect(erc721.connect(owner).safeMint(client1Address, 0))
            .to.emit(erc721, 'Minted').withArgs(client1Address, 0)
    ...
    it('transaction should be reverted if not owner try to mint', async () => {
        ...
        await expect(erc721.connect(client1).safeMint(client1Address, 0)).to.be.revertedWith('Access denied!')
    })
    ...
    it('transaction should be reverted if try to mint token that already exist', async () => {
        ...
        await expect(erc721.connect(owner).safeMint(client1Address, 0)).to.be.revertedWith('Token already exist!')
    })
    ...
    it('should be possible to approve access to token by token owner', async () => {
        ...
        await expect(erc721.approve(client1Address, 0))
            .to.emit(erc721, 'Approval').withArgs(ownerAddress, client1Address, 0)
    ...
    it('transaction should be reverted if not owner try to approve access to token', async () => {
        ...
        await expect(erc721.connect(client1).approve(client1Address, 0)).to.be.revertedWith('Access denied!')
    })
    ...
    it('should be possible to set approve for all to true', async () => {
        ...
        await expect(erc721.connect(owner).setApprovalForAll(client1Address, true))
            .to.emit(erc721, 'ApprovalForAll').withArgs(ownerAddress, client1Address, true)
        expect(await erc721.isApprovedForAll(ownerAddress, client1Address)).to.eq(true)
    })
    ...
    it('should be possible to set approve for all to false', async () => {
        ...
        await expect(erc721.connect(owner).setApprovalForAll(client1Address, false))
            .to.emit(erc721, 'ApprovalForAll').withArgs(ownerAddress, client1Address, false)
        expect(await erc721.isApprovedForAll(ownerAddress, client1Address)).to.eq(false)
    })
    ...
    it('should be possible to transfer token by owner', async () => {
        ...
        await expect(erc721.connect(owner).safeTransferFrom(ownerAddress, client1Address, 0))
            .to.emit(erc721, 'Transfer').withArgs(ownerAddress, client1Address, 0)
        expect(await erc721.ownerOf(0)).to.eq(client1Address)
        expect(await erc721.balanceOf(client1Address)).to.eq(1)
    ...
    it('should be possible to transfer token by operator who has access for all', async () => {
        ...
        await expect(erc721.connect(client1).safeTransferFrom(ownerAddress, client1Address, 0))
            .to.emit(erc721, 'Transfer').withArgs(ownerAddress, client1Address, 0)
        expect(await erc721.ownerOf(0)).to.eq(client1Address)
        expect(await erc721.balanceOf(client1Address)).to.eq(1)
        expect(await erc721.balanceOf(ownerAddress)).to.eq(0)
    ...
    it('transaction should be reverted if try to transfer token without approve', async () => {
        ...
        await erc721.connect(owner).safeMint(ownerAddress, 0)
        await expect(erc721.connect(client1).safeTransferFrom(ownerAddress, client1Address, 0)).to.be.revertedWith('Access denied!')
    })
})
